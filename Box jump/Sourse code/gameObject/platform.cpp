...
Platform::Platform()
{
	...
	m_material.density = dataManager->getObjectsSettgins()->get<float>("Platform.Density");
	m_material.restitution = dataManager->getObjectsSettgins()->get<float>("Platform.Restitution");
	m_material.staticFriction = dataManager->getObjectsSettgins()->get<float>("Platform.StaticFriction");
	m_material.dynamicFriction = dataManager->getObjectsSettgins()->get<float>("Platform.DynamicFriction");
}
...
void Platform::initialize()
{
	...
	if (m_state & PlatformState_Broken)
		m_sprite->setTextureRect(DataManager::instance()->getSpriteContainer("BrokenPlatformSprite")->rect);
	else
		m_sprite->setTextureRect(DataManager::instance()->getSpriteContainer("PlatformSprite")->rect);
}
